...
import { StockComponentsPage, StockDeleteDialog, StockUpdatePage } from './stock-farmatify.page-object';
...
describe('Stock e2e test', () => {
    ...
    it('should create and save Stocks', async () => {
        ...
        await stockComponentsPage.clickOnCreateButton();
        await promise.all([
            stockUpdatePage.setUnidadesInput('5'),
            stockUpdatePage.setFechaRepuestaInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            stockUpdatePage.farmaciaSelectLastOption()
        ]);
        expect(await stockUpdatePage.getUnidadesInput()).to.eq('5');
        expect(await stockUpdatePage.getFechaRepuestaInput()).to.contain('2001-01-01T02:30');
    ...
    it('should delete last Stock', async () => {
        ...
        expect(await stockDeleteDialog.getDialogTitle()).to.eq('farmatifyApp.stock.delete.question');
